...
from models.mca import MCAGate, StdPool, MCALayer
...
class PSPPCSPC(nn.Module):
    # CSP https://github.com/WongKinYiu/CrossStagePartialNetworks
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5, k=(5, 9, 13)):
        super(PSPPCSPC, self).__init__()
        c_ = int(2 * c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = PConv(c_, c_)
        self.cv4 = Conv(c_, c_, 1, 1)
        self.m = nn.ModuleList([nn.MaxPool2d(kernel_size=x, stride=1, padding=x // 2) for x in k])
        self.cv5 = Conv(4 * c_, c_, 1, 1)
        self.cv6 = Conv(c_, c_)
...
class V7DownSampling_Neck_ATT(nn.Module):
    def __init__(self, inc, ouc) -> None:
        super(V7DownSampling_Neck_ATT, self).__init__()

        self.maxpool = nn.Sequential(
            nn.MaxPool2d(kernel_size=2, stride=2),
            Conv(inc, ouc, k=1)
        )
        self.att = CBAMBlock(ouc)
        self.conv = nn.Sequential(
            Conv(inc, ouc, k=1),
            self.att(Conv(ouc, ouc, k=3, s=2)),
        )
...
class Yolov7_E_ELAN_PC(nn.Module):

    def __init__(self, inc, ouc, hidc, act=True):
        super(Yolov7_E_ELAN_PC, self).__init__()
...
class Yolov7_E_ELAN_RFAConv(nn.Module):

    def __init__(self, inc, ouc, hidc, act=True, stride=1):
        super(Yolov7_E_ELAN_RFAConv, self).__init__()

        self.conv1 = Conv(inc, ouc, k=1)
        self.conv2 = Conv(inc, ouc, k=1)
        self.conv3 = RFAConv(ouc, hidc, kernel_size=3)
        self.conv4 = RFAConv(hidc, hidc, kernel_size=3)
        self.conv5 = RFAConv(hidc, hidc, kernel_size=3)
        self.conv6 = RFAConv(hidc, hidc, kernel_size=3)
        self.conv7 = Conv(hidc * 4, ouc, k=1)
...
class Yolov7_E_ELAN_PC_ATT(nn.Module):
    def __init__(self, inc, ouc, hidc, act=True):
        super(Yolov7_E_ELAN_PC_ATT, self).__init__()
    ...
    def forward(self, x):
        ...
        x_final = self.att(self.conv7(x_concat))
        return x_final
...
class Yolov7_E_ELAN_NECK(nn.Module):
    def __init__(self, inc, ouc, hidc, act=True):
        ...
        self.conv5 = PConv(hidc, hidc, k=3, act=act)
        self.conv6 = PConv(hidc, hidc, k=3, act=act)
...
class Yolov7_E_ELAN_NECK_DySnake(nn.Module):
    def __init__(self, inc, ouc, hidc, act=True):
        super(Yolov7_E_ELAN_NECK_DySnake, self).__init__()

        self.conv1 = Conv(inc, ouc, k=1, act=act)
        self.conv2 = Conv(inc, ouc, k=1, act=act)
        self.conv3 = Conv(ouc, hidc, k=3, act=act)
        self.conv4 = DySnakeConv(hidc, hidc, k=3, act=act)
        self.conv5 = DySnakeConv(hidc, hidc, k=3, act=act)
        self.conv6 = DySnakeConv(hidc, hidc, k=3, act=act)
        self.conv7 = Conv(hidc * 4 + ouc * 2, ouc, k=1, act=act)
...
class Yolov7_E_ELAN_NECK_pc(nn.Module):
    def __init__(self, inc, ouc, hidc, act=True):
        super(Yolov7_E_ELAN_NECK_pc, self).__init__()
